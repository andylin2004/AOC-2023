def sort_by_power(cards):
    ...
    # one
        ...
        return 2
    # two
    elif len(inventory) == 3 and set(inventory.values()) == {1, 2, 2}:
        return 3
    # three of a kind
    elif len(inventory) == 3 and set(inventory.values()) == {1, 1, 3}:
        return 4
    elif len(inventory) == 2 and set(inventory.values()) == {12, 3}:
        return 4
    # full house
    elif len(inventory) == 2 and set(inventory.values()) == {2,3}:
        return 5
    # four of a kind
    elif len(inventory) == 2 and set(inventory.values()) == {1, 4}:
        return 6
    # five of a kind
