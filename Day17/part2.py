...
def dij(array): #dijkastra algo for pathcounting
    ...
    # three arrays
    results = [[[[10000 for _ in range(11)] for _ in range(4)] for _ in range(len(array[0]))] for _ in range(len(array))] # for keeping the least number of steps to get to an array slot
    isVisited = [[[[False for _ in range(11)] for _ in range(4)] for _ in range(len(array[0]))] for _ in range(len(array))] # one for keeping track of if we already determined the least number of steps to get to an array slot
    ...
    results[0][0] = [[0] * 11 for _ in range(4)] # preprocessing the start, as it doesn't count at all and we don't want anything bad to happen
    ...
    while len(pq) > 0:
        ...
        for cx,cy in zip(dx,dy):
            if 0 <= x+cx < len(array) and 0 <= y+cy < len(array[0]):
                ...
                if 0 <= dir <= 1 and 0 <= abs(x_ctr) <= 10:
                    newDist = results[x][y][dir][abs(x_ctr)]
                elif 2 <= dir <= 3 and 0 <= abs(y_ctr) <= 10:
                ...
                if 0 <= dir <= 1 and 0 <= abs(x_ctr+cx)<= 10:
                    checkMax = (x+cx, y+cy, dir, abs(x_ctr+cx))
                    # print("max", checkMax)
                elif 2 <= dir <= 3 and 0 <= abs(y_ctr+cy)<= 10:
                ...
                if results[checkMax[0]][checkMax[1]][checkMax[2]][checkMax[3]] > newDist: # we want the least num of steps to the neighbor as possible, so keep the min of the two for the tentative result
                    ...
                    if abs(x_ctr) == 0 and abs(cx) == 1 and x_ctr * cx >= 0 and ((cx == -1 and 0 <= x-4) or (cx == 1 and x+4 < len(array))):
                        for i in range(0, cx*4, cx):
                            results[checkMax[0]+i][checkMax[1]][checkMax[2]][checkMax[3]+abs(i)] = newDist
                            if i != cx*3:
                                newDist += array[checkMax[0]+i+cx][checkMax[1]]
                            else:
                                heapq.heappush(pq, [newDist, (x+cx*4, y+cy), (x_ctr + cx*4, 0)])

                            isVisited[checkMax[0]+i][checkMax[1]][checkMax[2]][checkMax[3]+abs(i)] = True
                    elif abs(y_ctr) == 0 and abs(cy) == 1 and y_ctr * cy >= 0 and ((cy == -1 and 0 <= y-4) or (cy == 1 and y+4 < len(array[0]))):
                        for i in range(0, cy*4, cy):
                            results[checkMax[0]][checkMax[1]+i][checkMax[2]][checkMax[3]+abs(i)] = newDist
                            if i != cy*3:
                                newDist += array[checkMax[0]][checkMax[1]+i+cy]
                            else:
                                heapq.heappush(pq, [newDist, (x+cx, y+cy*4), (0, y_ctr+cy*4)])
                            
                            isVisited[checkMax[0]][checkMax[1]+i][checkMax[2]][checkMax[3]+abs(i)] = True
                    elif 4 <= abs(x_ctr) < 10 and abs(cx) == 1 and x_ctr * cx >= 0 and (y_ctr == 0 or 4 <= y_ctr <= 10):
                        # print(x_ctr, "x")
                    ...
                    elif 4 <= abs(y_ctr) < 10 and abs(cy) == 1 and y_ctr * cy >= 0 and (x_ctr == 0 or 4 <= x_ctr <= 10):
                        # print(y_ctr, "y")
                        results[checkMax[0]][checkMax[1]][checkMax[2]][checkMax[3]] = newDist
